...
def convert_line(line)
  ...
  when 'SyncTrack'
    case line.strip
    when /(.*) = B (.*)/
      $rmxpchart_file["Sync"][($1.to_s.strip.to_i / 10)] = ($2.to_s.strip.to_i / 1000)
    end
  ...
  when 'EVENTS'
    ...
    when /(.*) = E "(.*)"/
      if !$rmxpchart_file["Events"][($1.to_s.strip.to_i / 10)].is_a?(Array)
        $rmxpchart_file["Events"][($1.to_s.strip.to_i / 10)] = [$3.to_s.strip]
      else
        $rmxpchart_file["Events"][($1.to_s.strip.to_i / 10)].push($3.to_s.strip)
      end
    end
    $rmxpchart_file["Last"] = $1.to_i if $1.to_i > $rmxpchart_file["Last"]
  when 'Expert'
    ...
    when /(.*) = S 2 (.*)/
      if !$rmxpchart_file["ExpertSpecial"][($1.to_s.strip.to_i / 10)].is_a?(Array)
        $rmxpchart_file["ExpertSpecial"][($1.to_s.strip.to_i / 10)] = [[$2.to_s.strip.to_i,$3.to_s.strip.to_i]]
      else
        $rmxpchart_file["ExpertSpecial"][($1.to_s.strip.to_i / 10)].push([$2.to_s.strip.to_i,$3.to_s.strip.to_i])
      end
    end
    $rmxpchart_file["Last"] = $1.to_i if $1.to_i > $rmxpchart_file["Last"]
  when 'Hard'
