puts '#=============================================================================='
puts '# RPG Maker XP Powerful Protection [Custom:484026]'
puts '# By: PedroHLC'
puts '# Ver: 0.7.5 06/02/11'
puts '#------------------------------------------------------------------------------'
...
module RGSSPP
  module_function 
  def encrypt_str(str)
    pass = (((str.size/2)**2 + 484026)/2).to_s
    i = 0
    while i <= (str.size - 1)
...
def encrypt_from(path)
  ...
  rxdata_names = Dir.entries(path).find_all{|filename| filename.include?('.rxdata')}
