...
def load_rgss_scripts(scripts_fname='Data/Scripts.rxdata')
  $game_loading = Sprite.new
  $game_loading.bitmap = Bitmap.new(640,480)
  cx = $game_loading.bitmap.text_size('Loading...').width
  $game_loading.bitmap.draw_text(320 - (cx / 2), 224, cx, 32, 'Loading...', 2)
  begin
    $PRE_SCRIPTS = Marshal.load(rgssp_desencrypt(File.open('Data/PreScripts.rxdata', 'rb').readlines.to_s))
