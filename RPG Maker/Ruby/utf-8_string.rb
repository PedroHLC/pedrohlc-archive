...
str = "åuvres ComplËtes"
str = "åuvres \000ComplËtes"
...
puts "‡cA?fÈ‡".swapcase_utf8
puts "‡cA?fÈ‡".swapcase_utf8!

puts
puts str.slice_utf8(/../i)
puts str.slice_utf8(/(.).*?\1/i)
puts "‡¿".slice_utf8(/(.).*?\1/i)   # => nil despite the i option!
puts "aA".slice(/(.).*?\1/i)        # => aA
puts "‡¿ ‡¿".slice_utf8!(/([‡¿]).*?\1/i)
puts "‡¿ ‡¿".slice_utf8!(/(.).*?\1/ium)
puts "? ‡¿ ? ‡¿".slice_utf8!(/(.).*?\1/ium)
...
puts
p str.scan_utf8(/./)
"‡cA?fÈ‡".scan_utf8(/./) { |c| puts c }
"‡cA?fÈ‡".scan_utf8(/(.)(.)?/) { |a,b| print a,b,"\n" }

puts
p "‡cA?fÈ‡".index_utf8('?')
p "‡cA?fÈ‡".index_utf8('?f')
p "‡cA?fÈ‡".index_utf8('z')
p "kfÈ‡ ?f ‡c ? 9h?!fz A?kfÈ‡ ?f 9?!fz".index_utf8('9?!fz')
p "kfÈ‡ ?f ‡c ? 9h?!fz A?kfÈ‡ ?f 9?!ofz 9?!fz".index_utf8(/9?!fz/)
p "kfÈ‡ ?f ‡c ? 9?!fz A?kfÈ‡ ?f 9?!ofz 9?!fz kfÈ‡ ?f ‡c ? 9h?!fz 9?!fz A?kfÈ‡ ?f".index_utf8(//)

puts
p "kfÈ‡ ?f ‡c ? 9?!fz A?kfÈ‡ ?f 9?!ofz 9?!fz kfÈ‡ ?f ‡c ? 9h?!fz 9?!fz A?kfÈ‡ ?f".rindex_utf8('9?!fz')
p "kfÈ‡ ?f ‡c ? 9?!fz A?kfÈ‡ ?f 9?!ofz 9?!fz kfÈ‡ ?f ‡c ? 9h?!fz 9?!fz A?kfÈ‡ ?f".rindex_utf8(/9?!fz/)
p "kfÈ‡ ?f ‡c ? 9?!fz A?kfÈ‡ ?f 9?!ofz 9?!fz kfÈ‡ ?f ‡c ? 9h?!fz 9?!fz A?kfÈ‡ ?f".rindex_utf8(/9..fz/)
p "kfÈ‡ ?f ‡c ? 9?!fz A?kfÈ‡ ?f 9?!ofz 9?!fz kfÈ‡ ?f ‡c ? 9h?!fz 9?!fz A?kfÈ‡ ?f".rindex_utf8(//)

puts
puts "‡cA?fÈ‡".utf8_to_utf16le.utf16le_to_utf8
puts "‡cA?fÈ‡".utf8_to_utf16le.utf16le_to_utf8.asciify_utf8
puts "‡¿".slice_utf8(/../i)
puts "‡¿".slice_utf8!(/../i)
...
puts "? ‡¿ ? ‡¿".count_utf8('z')
puts "? ‡¿/ ^? ‡¿".count_utf8('/?^')
puts "? ‡¿/ ^? ‡¿".count_utf8('^/?^')  # count all chars except those specified; note that the leading ^ will result in the regex: /[^\/?^]/u
...
puts "? ‡¿ ? ‡¿".starts_with_utf8?('?')
puts "? ‡¿ ? ‡¿".ends_with_utf8?('k')
puts "".ends_with_utf8?('k')
puts "? ‡¿ ? ‡¿".ends_with_utf8?('')
puts "? ‡¿ ? ‡".starts_with_utf8?('? ‡¿ ? ‡¿')

puts "? ‡¿ ? ‡".insert_utf8(20, "abc")
puts "?‡¿?‡".insert_utf8(2, "abc")
puts "?‡¿?‡".insert_utf8(-2, "abc")
puts "?‡¿?‡".insert_utf8(-200, "abc")
puts "?‡¿?‡".insert_utf8(200, "abc")
...
puts "?‡¿?‡??".utf8_to_unicode.unicode_to_utf8
puts "?‡¿?‡??".size_utf8
...
# see:
# - http://intertwingly.net/stories/2004/04/14/i18n.html (IÒtÎrn‚tiÙn‡lizÊti¯n)
# - http://www.intertwingly.net/blog/1763.html (Unicode and weblogs)
# - http://www.intertwingly.net/blog/1768.html (UTF-8 musings)

puts "IÒtÎrn‚tiÙn‡lizÊti¯n".asciify_utf8
puts "IÒtÎrn‚tiÙn‡lizÊti¯n".utf8_to_unicode
puts "IÒtÎrn‚tiÙn‡lizÊti¯n".utf8_to_unicode.unicode_to_utf8
puts "IÒtÎrn‚tiÙn‡lizÊti¯n".size_utf8
puts "IÒtÎrn‚tiÙn‡lizÊti¯n".upcase_utf8
...
p "IÒtÎrn‚tiÙn‡lizÊti¯\x00n".utf8_to_dec
puts "IÒtÎrn‚tiÙn‡lizÊti¯\x00n".utf8_to_dec
p "IÒtÎrn‚tiÙn‡lizÊti¯\x00n".utf8_to_dec.dec_to_utf8
puts "IÒtÎrn‚tiÙn‡lizÊti¯\x00n".utf8_to_dec.dec_to_utf8

puts
p "IÒtÎrn‚tiÙn‡lizÊti¯\x00n".utf8_to_hex
puts "IÒtÎrn‚tiÙn‡lizÊti¯\x00n".utf8_to_hex
p "IÒtÎrn‚tiÙn‡lizÊti¯\x00n".utf8_to_hex.hex_to_utf8
puts "IÒtÎrn‚tiÙn‡lizÊti¯\x00n".utf8_to_hex.hex_to_utf8
    
puts
p "IÒtÎrn‚tiÙn‡lizÊti¯\x00n".utf8_to_oct
puts "IÒtÎrn‚tiÙn‡lizÊti¯\x00n".utf8_to_oct
p "IÒtÎrn‚tiÙn‡lizÊti¯\x00n".utf8_to_oct.oct_to_utf8
puts "IÒtÎrn‚tiÙn‡lizÊti¯\x00n".utf8_to_oct.oct_to_utf8

puts
puts '"Hello, world" in Portuguese: "Ol· Mundo" or "AlÙ Mundo" (PortuguÍs)'.email_subject_utf8
