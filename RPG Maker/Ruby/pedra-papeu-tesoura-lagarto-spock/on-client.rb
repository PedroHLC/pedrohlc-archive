...
print "Porta: "
port = gets.to_i
puts "Conectando em #{host}:#{port}"

$player = $server = TCPSocket.open(host, port)
puts "\nConectado com sucesso em #{host}:#{port}"
...
puts "Servidor online, começando o jogo...\n"
loop do
  result = play
  if result == 'off'
    break
