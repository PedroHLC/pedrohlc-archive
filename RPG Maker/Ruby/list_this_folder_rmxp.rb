=begin
RGSS103J )RPG Maker XP( Descompiled Data
by: PedroHLC
=end
module RPG
class Map
...
class Troop
  class Page
    def initialize
      ...
      @span = 0;@list = [RPG::EventCommand.new]
    end
    attr_accessor :condition
    attr_accessor :span
    attr_accessor :list
...
=begin
List Folder + Marhsal Detector + RMXP Data
by: PedroHLC
=end
...
def list_folder(path)
  for item in Dir.entries(path)
    ...
    if File.directory?(item_path)
      ...
      $file_list.push([item_path, File.size(item_path).to_s])
    end
  end
end
...
for i in 0..($file_list.size - 1)
  file = $file_list[i]
  begin
    fileF = File.open(file[0], "rb")
    test = Marshal.load(fileF)
    output_file.write("#{file[0]} :> #{file[1]} - marshal\n")
  rescue Exception => mini_error
