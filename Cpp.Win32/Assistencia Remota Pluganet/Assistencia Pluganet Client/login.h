			...
			if(packet[0] == 'l'){
				...
				if(packet[2] != '-'){
					...
					if(tickets > 0){
						...
						this->textLogin->ReadOnly = false;
						this->textPass->ReadOnly = false;
						this->labelStatus->Text = L"Compre mais tickets ou utilize outra conta!";
						this->groupMain->UseWaitCursor = false;
						this->worked = false;
						showalert(L"Você também não pode ser cliente sem comprar tickets!");
					}else{
						mainConnSend("\n");
						this->buttonGo->Enabled = true;
						this->textLogin->ReadOnly = false;
						...
						this->groupMain->UseWaitCursor = false;
						this->worked = false;
						showalert(L"Você não tem tickets suficientes para inciar esta chamada!");
					}
					return;
				}else{
					this->buttonGo->Enabled = true;
